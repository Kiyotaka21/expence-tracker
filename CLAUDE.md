...
**Заголовок PR — по правилам заголовка коммита** (скилл [commit](.claude/skills/commit/SKILL.md)): `<тип>(<область>): <что делает>`, до 72 символов, императив, с маленькой буквы, без точки в конце. Область — по преобладающей части диффа: PR, где заодно правились сиды и документация, но основная работа в вебе, остаётся `feat(web)`, а не теряет область.
