...
- **Доступ в интернет при сборке веба**: шрифт Geist подключён через `next/font/google` и
  скачивается на этапе сборки. `next build` без сети упадёт.
...
единицы работы, проверки, сообщение, пуш.
