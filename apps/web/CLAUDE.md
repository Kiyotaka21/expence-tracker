...
Общее для всего репозитория — ветки, коммиты, pull request, контракты в `packages/contracts` — в корневом [CLAUDE.md](../../CLAUDE.md). Здесь только то, что нужно при правках веба.
