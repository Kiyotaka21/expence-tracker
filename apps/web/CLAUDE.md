...
Фронтенд целиком переведён на Feature-Sliced Design и shadcn/ui (`radix-nova`). Оболочка приложения — тёмная рельса навигации и скруглённая панель содержимого на градиентном холсте: виджет `app-nav`, разметка в `src/app/(dashboard)/layout.tsx`. Экраны входа и регистрации повторяют ту же панель, разрезанную надвое. Формы собраны на `Field` + `react-hook-form` + контрактные схемы; остальные экраны собраны в слоях.

`/dashboard` — главный экран: приветствие с выбором валюты и месяца, сводка месяца (`month-summary` — баланс крупно, полоса «израсходовано % доходов» и две пастельные плитки), кольцо расходов по категориям (`expense-breakdown`) и список операций с фильтрами и пагинацией по 10 записей. Табов «Транзакции / Категории» на нём больше нет: разделы открываются из рельсы. Списки операций и категорий вынесены в виджеты — те же самые показывают `/transactions` и `/categories`.

В интерфейсе одна лексика: то, что раньше называлось транзакцией, везде «операция» — и в навигации, и в заголовках, и в тостах.

Тестов и тестового раннера в вебе нет: Vitest заведён только в `apps/api`. Первый тест здесь заведёт его и тут — процедура в скилле [test](../../.claude/skills/test/SKILL.md), тестам компонентов дополнительно нужны `jsdom` и Testing Library. В интерфейсе нет переключателя темы: тема следует настройке ОС.

Вёрстку после правок проверяет скилл [ui-check](../../.claude/skills/ui-check/SKILL.md) (`/ui-check [маршрут]`): Playwright MCP по живому `next dev`, кадры на 1440 и 390 плюс границы `lg` и `sm`, на которых переключается оболочка. Горизонтальный скролл он ловит числом, а не глазами — на скриншоте лишние пиксели не видны.
...
| `widgets`  | `app-nav` (рельса, мобильная шапка, марка), `month-summary`, `expense-breakdown` (кольцо + легенда), `transaction-list` (фильтры + пагинация), `category-list`, `create-menu` |
...
Компоненты живут не в зависимостях, а в коде: куда их писать, описано в `apps/web/components.json`. Стиль — `radix-nova` (Radix UI + lucide), но палитра и типографика переопределены целиком — см. [Дизайн-система](#дизайн-система). Своего набора UI-примитивов (бывший `src/components/ui`) больше нет.

Примитивы подстроены под эту палитру, и `shadcn add --overwrite` эти правки снесёт: у `button` подняты высоты (`default` — 36px, `lg` — 44px) и добавлен `font-heading`, у `input`, `native-select` и `input-group` высота 40px и фон `bg-card`, у `card` вместо `ring` рамка и шаг `--card-spacing: 5`, у `dialog` радиус `2xl` и padding 5.
...
- **Шрифты Inter и Manrope скачиваются на этапе сборки** (`next/font/google` в `layout.tsx`) с подмножеством `cyrillic` — без него русский текст рисовался бы фолбэком. `next build` без сети упадёт.
...
Все токены лежат в `src/app/globals.css`, компоненты цвета не хардкодят: правите вид — правьте токен, а не класс в разметке.

| Группа      | Токены                                                 | Про что                                                                                        |
| ----------- | ------------------------------------------------------ | ---------------------------------------------------------------------------------------------- |
| Поверхности | `--background` + `--canvas-image`, `--card`, `--muted` | холст под панелью — градиент; панель и карточки — `card`; врезки вроде блока баланса — `muted` |
| Акценты     | `--primary` (лазурь), `--tint-blue`, `--tint-violet`   | primary — действия и активные состояния, тинты — пастельные плитки сводки                      |
| Деньги      | `--income`, `--expense`                                | зелёный и красный, читаемые как текст и на светлом, и на тёмном                                |
| Рельса      | `--sidebar*`, включая своё `--sidebar-muted`           | рельса тёмная **всегда**: в `.dark` эти токены не переопределяются                             |
| Графики     | `--chart-1`…`--chart-6`                                | категориальные слоты, проверенные набором                                                      |

Ещё два добавления к реестру: `font-heading` разрешается в Manrope (заголовки, навигация, суммы; текст и контролы — Inter), а утилита `label-micro` даёт микроподпись капителью — шапки таблиц и заголовок раздела в рельсе.

- **Слоты `--chart-*` нельзя менять по одному.** Набор и его порядок прогнаны валидатором палитр: полоса светлоты, порог насыщенности, различимость соседних слотов при дальтонизме, контраст к поверхности — отдельно для светлой и тёмной темы. Поменяли цвет или порядок — перепрогоняйте проверку целиком.
- **Цвет дуги в кольце — цвет категории из БД**, слот достаётся только категориям без своего цвета: одна сущность не может быть двух цветов, тот же цвет стоит у её иконки в списках. Опознавать дугу по цвету при этом не нужно — каждую называет строка легенды с точной суммой.
- **Библиотеки графиков нет и не нужно.** Кольцо — `shared/ui/donut.tsx`: окружности со `stroke-dasharray`, зазор в 2 единицы вместо обводки, наведение подменяет значение в центре. `chart` из реестра тянет recharts, а рисуем мы одно кольцо.
- **Смещения дуг считаются заново на каждой** (`slices.slice(0, index).reduce(...)`): накапливать сумму в переменной внутри `map` запрещает React Compiler — правило `react-hooks/immutability`.
...
Наружу сумма идёт со знаком валюты, а не с кодом: `formatAmount(amount, currency)` — «1 200,00 ₽». `formatCompactAmount` («53 тыс. ₽») только для крупного кегля внутри графика, где точное значение стоит рядом в легенде.

Из этого правила ровно два исключения, и оба — геометрия, а не деньги: доля дуги в кольце и процент в полосе «израсходовано». Числа для них берутся из готовых сумм сервера, наружу как деньги не показываются, и суммировать `Decimal` ради подписи всё так же нельзя — поэтому у серой дуги «остальные категории» суммы в легенде нет.
