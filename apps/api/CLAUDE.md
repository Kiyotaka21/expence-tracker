...
Общее для всего репозитория — ветки, pull request, контракты в `packages/contracts` — в корневом [CLAUDE.md](../../CLAUDE.md), процедуры коммита и PR — в скиллах [commit](../../.claude/skills/commit/SKILL.md) и [pr](../../.claude/skills/pr/SKILL.md). Здесь только то, что нужно при правках бэкенда.
